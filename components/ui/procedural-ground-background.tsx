...
const ProceduralGroundBackground: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        // Color Palette
        vec3 baseColor = vec3(0.01, 0.01, 0.05); // Deeper Space
        vec3 accentColor = vec3(0.1, 0.4, 0.9);   // Brighter Electric Blue
        vec3 neonColor = vec3(0.7, 0.3, 1.0);     // Vibrant Neon Purple
        
        // Composite
        vec3 finalColor = mix(baseColor, accentColor, n * 0.5);
        finalColor += topoLine * neonColor * depth * 0.6;
        
        // Horizon Fog / Fade (Visible at bottom, fades at top)
        float fade = smoothstep(-1.0, 0.5, uv.y);
        finalColor *= (1.0 - length(uv) * 0.2) * (1.1 - fade);
  ...
  return (
    <div className="absolute inset-0 w-full h-full bg-zinc-950 z-0 overflow-hidden pointer-events-none">
      <canvas
        ref={canvasRef}
